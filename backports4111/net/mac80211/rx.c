				 ...
				 int rtap_len, bool has_fcs)
{
	...
	if (local->hw.flags & IEEE80211_HW_SIGNAL_DBM &&
		...
		*pos = status->signal;
		printk("Trama: %s, %d", __FUNCTION__, status->signal);
		rthdr->it_present |=
					    ...
					    struct sk_buff *skb, bool consume)
{
	...
	n=sprintf(dbuf, "%u,%u,%u,%u,%u,%u", dst[0],dst[1],dst[2],dst[3],dst[4],dst[5]);
	n=sprintf(sbuf, "%u,%u,%u,%u,%u,%u", src[0],src[1],src[2],src[3],src[4],src[5]);
	printk("TRAMA: dBm signal power: %d -- addr1: %s -> %s \n", status->signal, sbuf, dbuf);
	// TRAMA----------------------
