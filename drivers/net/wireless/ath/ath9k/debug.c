				  ...
				  size_t count, loff_t *ppos)
{
    ...
    struct ath_softc *sc = context->priv;
	char buf[512];
	unsigned int len, val;

	// FIXME: Is the wakeup/restore call needed?
	ath9k_ps_wakeup(sc);
	val = timefunc->getter(sc->sc_ah);
	ath9k_ps_restore(sc);
				   ...
				   size_t count, loff_t *ppos)
{
	...
	// FIXME: Is the wakeup/restore call needed?
	ath9k_ps_wakeup(sc);
	timefunc->setter(sc->sc_ah, (u32)val);
    ath9k_ps_restore(sc);
...
static ssize_t read_file_nocsma(struct file *file, char __user *user_buf,
				  size_t count, loff_t *ppos)
{
    struct ath_softc *sc = file->private_data;
	char buf[512];
    ...
    ath9k_ps_wakeup(sc);
	val = ath9k_hw_get_csma_flag(sc->sc_ah);    
    printk("Trama %s AR_DIAG_SW=0x%08x\n", __FUNCTION__, REG_READ(sc->sc_ah, AR_DIAG_SW));
    ath9k_ps_restore(sc);    
    len = sprintf(buf, "0x%08x\n", val);
...
static ssize_t write_file_nocsma(struct file *file, const char __user *user_buf,
				   size_t count, loff_t *ppos)
{
	struct ath_softc *sc = file->private_data;
	unsigned long val;
    ...
    ath9k_ps_wakeup(sc);
    if(val==1)
        ath9k_hw_set_csma_flag(sc->sc_ah);
    else
        ath9k_hw_reset_csma_flag(sc->sc_ah);
    printk("Trama %s AR_DIAG_SW=0x%08x\n", __FUNCTION__, REG_READ(sc->sc_ah, AR_DIAG_SW));

	return count;
}

static const struct file_operations fops_no_csma = {
	.read = read_file_nocsma,
	.write = write_file_nocsma,
	.open = simple_open,
	.owner = THIS_MODULE,
	.llseek = default_llseek,
};

static ssize_t read_file_csmareg(struct file *file, char __user *user_buf,
				  size_t count, loff_t *ppos)
{
    struct ath_softc *sc = file->private_data;
	char buf[512];
	...
	len = snprintf(buf, sizeof(buf), "%s\nValue: 0x%08X = %d\n",
		"disabling CSMA", val, val);
        
	return simple_read_from_buffer(user_buf, count, ppos, buf, len);
}

static const struct file_operations fops_csmareg = {
	.read = read_file_csmareg,
	.open = simple_open,
	.owner = THIS_MODULE,
	.llseek = default_llseek,
};
...
// TRAMA
    debugfs_create_file("no_csma", S_IRUSR | S_IWUSR,
			    sc->debug.debugfs_phy, sc, &fops_no_csma);
    debugfs_create_file("csma_reg", S_IRUSR,
			    sc->debug.debugfs_phy, sc, &fops_csmareg);
